...
def get_certificates_to_provision(env, limit_domains=None, show_valid_certs=True):
	...
	actual_web_domains = get_web_domains(env, exclude_dns_elsewhere=False)
	...
	for domain in plausible_web_domains:
		...
		# Check that the DNS resolves to here.
			...
			for rtype, value in [("A", env["PUBLIC_IP"]), ("AAAA", env.get("PUBLIC_IPV6"))]:
				...
				if rtype == 'AAAA' and response != '[Not Set]':
					response = ipaddress.IPv6Network(response + '/56', strict=False)
					value = ipaddress.IPv6Network(value + '/56', strict=False)
				if response != value:
